...
//! ## Solving ODEs
//!
//! The simplest way to create a new problem is to use the [OdeBuilder] struct. You can set many
//! configuration options such as the initial time ([OdeBuilder::t0]), initial step size
//! ([OdeBuilder::h0]), relative tolerance ([OdeBuilder::rtol]), absolute tolerance
//! ([OdeBuilder::atol]), parameters ([OdeBuilder::p]) and equations ([OdeBuilder::rhs_implicit],
...
//! ## Initial state
//!
//! The solver state is held in [OdeSolverState], and contains a state vector, the gradient of the
//! state vector, the time, and the step size. The [OdeSolverProblem] class has a collection of
//! methods to create and initialise a new state for each solver ([OdeSolverProblem::bdf_state],
...
//! the [StateRef] or [StateRefMut] structs
//!
//! Note that many of the solver creation methods (e.g. [OdeSolverProblem::bdf]) will create and
//! initialise a new state for you, so you might not need to manually create a state in many cases.
//! However, if you want to have more control over the initialisation of the state, or you want to
//! reuse a state across multiple solvers, then you can create and initialise the state manually as
//! described above.
//!
//! ## The solver
//!
...
//! ## Custom ODE problems
//!
//! The [OdeBuilder] struct is the easiest way to create a problem, and can be used to create an
//! ODE problem from a set of closures or the DiffSL language. However, if this is not suitable for
//! your problem or you want more control over how your equations are implemented, you can use your
//! own structs to define the problem and wrap them in an [OdeSolverEquations] struct. See the
//! [OdeSolverEquations] struct for more information.
//!
...
//! `NaN`, using the fact that `NaN`s propagate through most operations. However, this method is
//! not foolproof and will fail if, for example, your jacobian function uses any control flow that
//! depends on the input vector. If this is the case, you can provide the jacobian matrix directly
//! by implementing the optional [NonLinearOpJacobian::jacobian_inplace] and the
//! [LinearOp::matrix_inplace] (if applicable) functions, or by providing a sparsity pattern using
//! the [NonLinearOpJacobian::jacobian_sparsity] and [LinearOp::sparsity] functions.
//!
//! ## Events / Root finding
//!
...
//! integrating with the solver, you can use the return value of [OdeSolverMethod::step] to check
...
//! problem and the sensitivity equations simultaneously and return the result. If you are manually
//! stepping the solver, you can use the [OdeSolverMethod::interpolate_sens] method to obtain the
//! sensitivity solution at a given time. Otherwise the sensitivity vectors are stored in the
//! [OdeSolverState] struct.
//!
//! ## Checkpointing
//!
//! You can checkpoint the solver at the current internal time [OdeSolverMethod::checkpoint]
//! method.
//!
//! ## Interpolation
//!
//! The [HermiteInterpolator] struct provides a way to interpolate a solution between a sequence of
//! steps. If the number of steps in your solution is too large to fit in memory, you can instead
//! use checkpointing to store the solution at a reduced set of times and dynamically interpolate
//! between these checkpoints using the [Checkpointing] struct (at the cost of recomputing the
//! solution between the checkpoints).  
//!
...
//! [OdeBuilder::out_rtol] methods. It can be removed from the error control by setting the
//! tolerances to `None`.
//!
//! ## Adjoint Sensitivity Analysis
//!
//! If you require the partial gradient of the output function with respect to the parameters and
//! your parameter vector is sufficiently large, then it is more efficient to use the adjoint
//! sensitivity method. This method uses a lagrange multiplier to derive a set of adjoint ode
//! equations that are solved backwards in time, and then used to compute the sensitivities of the
//! output function. Checkpointing is typically used to store the forward solution at a set of
//! times as theses are required to solve the adjoint equations.
//!
//! To provide the builder with the required equations, you can use the
//! [OdeBuilder::rhs_adjoint_implicit], [OdeBuilder::init_adjoint], and
//! [OdeBuilder::out_adjoint_implicit] methods, or your equations struct must implement the
//! [OdeEquationsAdjoint] trait.
//!
//! To obtain the adjoint solution, it is first required to generate a checkpointing struct using
//! either [OdeSolverMethod::solve_with_checkpointing] or
//! [OdeSolverMethod::solve_dense_with_checkpointing] methods, which will solve the forwards
//! problem. Then you can create an adjoint solver using the functions on the problem struct, for
//! example [OdeSolverProblem::bdf_solver_adjoint]. Once you have created your adjoint solver, then
//! use the [AdjointOdeSolverMethod::solve_adjoint_backwards_pass] method to solve the adjoint
//! equations backwards in time and obtain your solution. The gradients of your outputs wrt the
//! parameters are stored in the [StateRef::sg] field.
//!
//! If you wish to manually do the timestepping, then the best place to start is by looking at the
//! source code for the [AdjointOdeSolverMethod::solve_adjoint_backwards_pass] method. During the
//! solution of the forwards problem you will need to use checkpointing to store the solution at a
//! set of times, and you can see how this is done in the
//! [OdeSolverMethod::solve_with_checkpointing] method.
//!
//! ## Resets
//!
